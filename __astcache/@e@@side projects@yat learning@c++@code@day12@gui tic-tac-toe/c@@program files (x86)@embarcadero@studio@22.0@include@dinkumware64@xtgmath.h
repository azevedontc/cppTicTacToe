...
extern "C" _CHECK_RETURN CRTTYPE double FUN(_IN double); \
_CHECK_RETURN CRTTYPE float FUN(_IN float); \
_CHECK_RETURN CRTTYPE long double FUN(_IN long double); \
...
extern "C" _CHECK_RETURN CRTTYPE double FUN(_IN double, ARG2); \
_CHECK_RETURN CRTTYPE float FUN(_IN float, ARG2); \
_CHECK_RETURN CRTTYPE long double FUN(_IN long double, ARG2); \
template<class _Ty> inline \
	typename _STD enable_if< _STD is_integral<_Ty>::value, double>::type \
...
extern "C" _CHECK_RETURN CRTTYPE double CALL_OPT FUN( \
	_IN double, _IN double); \
_CHECK_RETURN CRTTYPE float CALL_OPT FUN( \
	_IN float, _IN float); \
_CHECK_RETURN CRTTYPE long double CALL_OPT FUN( \
	_IN long double, _IN long double); \
template<class _Ty1, \
	class _Ty2> inline \
...
_C_STD_BEGIN
extern "C" _CRTIMP2 double pow(double, double);
...
_GENERIC_MATH1X(frexp, _OUT int *, _CRTSPECIAL)
_GENERIC_MATH1X(ldexp, _IN int, _CRTSPECIAL)
...
extern "C" _CHECK_RETURN CRTTYPE RET FUN(_IN double); \
template<class _Ty> inline \
...
template<class _Ty1,
	...
	typedef typename _STD _Common_float_type<_Ty1,
		typename _STD _Common_float_type<_Ty2, _Ty3>::type>::type type;
	return (_Hypot3((type)_Left, (type)_Middle, (type)_Right));
	}
...
//_GENERIC_MATH3(hypot3, _CRTSPECIAL)	// hand crafted
...
_GENERIC_MATH1X(nexttoward, _IN long double, _CRTSPECIAL)
...
_GENERIC_MATH1X(scalbln, _IN long, _CRTSPECIAL)
_GENERIC_MATH1X(scalbn, _IN int, _CRTSPECIAL)
